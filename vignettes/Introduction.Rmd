...
sample_dna_combined <- qubit_merger(hs_data = sample_HS_dna, br_data = sample_BR_dna) %>% mutate(SampleWell = ReaderWell)
...
It is important to point out here that although it is often the case that ReaderWell = SampleWell, SampleWell may not necessarily correspond to ReaderWell. ReaderWell is meant to indicate the location of a plate reader measurement, while SampleWell is meant to indicate the physical location of the source DNA. In this way, there is flexibility to measure DNA from multiple plates on the same plate reader measurement. This may be the source of some confusion, but it is why I recommend creating a mapping file that can help bridge the gap between where a sample is actually located (SampleWell) and where it was measured (ReadeWell), if these are different. If they are the same, you can simply copy this information with `new_data <- data %>% mutate(SampleWell = ReaderWell)`, as has been done in the examples above. Otherwise, this information should be contained elsewhere.
