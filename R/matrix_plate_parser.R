...
matrix_plate_parser <- function(matrix_barcode_plate_scan, plate_name = NA) {
  ...
  if (utils::tail(unlist(strsplit(matrix_barcode_plate_scan, "\\.")), n = 1) == "csv") {
    tube_order <-
      readr::read_csv(
        ...
        skip = 0
      ) %>% dplyr::mutate(Well = paste0(LocationRow, LocationColumn), TubeBarcode = TubeCode, Plate = ifelse(is.na(plate_name), RackID, plate_name)) %>% dplyr::select(Plate, Well, TubeBarcode)
  ...
  #   dplyr::select(Plate, Well, TubeBarcode)
