...
#' @param size size of plate (default = 96, alternative is 384 - not tested yet)
...
measure_dna_concentration <- function(plate_reader_file, standards_plate_reader_file = plate_reader_file, standard_wells, dye_used, qubit_volume = 2, elution_volume = 100, size = 96, print_standard_curve = FALSE) {
  ...
  plate <- read_plate(plate_reader_file = plate_reader_file, size = size)

  standard_data <- read_plate(plate_reader_file = standards_plate_reader_file, size = size) %>%
  ...
  standards_plot <- ggplot2::ggplot(data = standard_data, ggplot2::aes_string(x = "Measurement", y = "`DNA in Standard`")) +
    ...
    ggplot2::annotate("text", x = 0, y = max(standard_data$`DNA in Standard`), hjust = 0, label = standards_info) + EJC_theme() +
